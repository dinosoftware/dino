...
export default function ArtistDetailScreen({ artistId }: ArtistDetailScreenProps) {
  ...
  const renderHeaderBackground = () => {
    ...
    if (backgroundStyle === 'dynamicColor' && coverArtUrl) {
      const bgColor = artistColors.background || theme.colors.background.primary;
      return (
        <LinearGradient
    ...
    if (backgroundStyle === 'gradient' && coverArtUrl) {
      ...
      return (
        <LinearGradient
          colors={
            isDark
              ? [artistColors.primary + '30', artistColors.secondary + '40', 'transparent']
              : [artistColors.primary + '15', artistColors.secondary + '20', 'transparent']
          }
          style={StyleSheet.absoluteFill}
          start={{ x: 0, y: 0 }}
          end={{ x: 0, y: 1 }}
